/*CREACION DE LA BASE DE DATOS DEL PROYECTO -- SERVICE MARKET*/
CREATE DATABASE SERVICE_MARKET
USE SERVICE_MARKET

-----------------------------------------------------------------------------------------------------------------------
/*CREACION DE LA TABLA CIUDADES*/
CREATE TABLE CIUDADES(
ID_CIUDAD INT PRIMARY KEY IDENTITY(1,1),
NOMBRE_CIUDAD VARCHAR(50) NOT NULL)

/*PROCEDIMIENTO ALMACENADO PARA CREAR REGISTROS DE LA TABLA CIUDADES*/
CREATE PROCEDURE CREAR_CIUDADES(
@NOMBRE_CIUDAD VARCHAR(50),
@MENSAJE_CIUDAD VARCHAR(100) OUTPUT)
AS
BEGIN
    IF EXISTS (SELECT * FROM CIUDADES WHERE NOMBRE_CIUDAD = @NOMBRE_CIUDAD)
    BEGIN
       SET @MENSAJE_CIUDAD = 'La ciudad "' + @NOMBRE_CIUDAD + '" ya se encuentra registrada.'
    END
    ELSE
    BEGIN
        INSERT INTO CIUDADES (NOMBRE_CIUDAD) VALUES (@NOMBRE_CIUDAD)
    END
END

/*PROCEDIMIENTO ALMACENADO PARA LEER REGISTROS DE LA TABLA CIUDADES*/
CREATE PROCEDURE LEER_CIUDADES
AS
BEGIN
	SELECT * FROM CIUDADES
END

EXEC LEER_CIUDADES

/*PROCEDIMIENTO ALMACENADO PARA ACTUALIZAR REGISTROS DE LA TABLA CIUDADES*/
CREATE PROCEDURE ACTUALIZAR_CIUDADES(
@ID_CIUDAD INT,
@NOMBRE_CIUDAD VARCHAR(50),
@MENSAJE_CIUDAD_ACT VARCHAR(100) OUTPUT)
AS
BEGIN
	 IF NOT EXISTS (SELECT * FROM CIUDADES WHERE NOMBRE_CIUDAD = @NOMBRE_CIUDAD AND ID_CIUDAD <> @ID_CIUDAD)
    BEGIN
        UPDATE CIUDADES SET NOMBRE_CIUDAD = @NOMBRE_CIUDAD WHERE ID_CIUDAD = @ID_CIUDAD;
        SET @MENSAJE_CIUDAD_ACT = 'La ciudad ha sido actualizada exitosamente.';
    END
    ELSE
    BEGIN
        SET @MENSAJE_CIUDAD_ACT = 'El nombre de la ciudad ya está en uso.';
    END
END

/*PROCEDIMIENTO ALMACENADO PARA BORRAR REGISTROS DE LA TABLA CIUDADES*/
CREATE PROCEDURE BORRAR_CIUDADES(
@ID_CIUDAD INT)
AS
BEGIN
	DELETE FROM CIUDADES WHERE ID_CIUDAD = @ID_CIUDAD
END

/*CREACION DE LA FUNCION PARA CONTAR EL NUMERO DE CIUDADES REGISTRADAS*/
CREATE FUNCTION NUMERO_CIUDADES()
RETURNS INT
AS
BEGIN
  DECLARE @TOTAL_CIUDADES INT;

  SELECT @TOTAL_CIUDADES = COUNT(*) FROM CIUDADES;

  RETURN @TOTAL_CIUDADES;
END;

SELECT dbo.NUMERO_CIUDADES() AS NumeroCiudades;

/*REINICIAR CONTADOR DEL ID*/
SELECT MAX(ID_CIUDAD) FROM CIUDADES;
DBCC CHECKIDENT (CIUDADES, RESEED, 10)

-----------------------------------------------------------------------------------------------------------------------

/*CREACION DE LA TABLA USUARIOS*/
CREATE TABLE USUARIOS(
ID_USUARIO INT PRIMARY KEY IDENTITY(1,1),
NOMBRE_COMPLETO_USU VARCHAR(100),
CELULAR_USU VARCHAR(20) NOT NULL,
ID_CIUDAD_FK INT REFERENCES CIUDADES(ID_CIUDAD),
CORREO_ELECTRONICO_USU VARCHAR(100) NOT NULL,
CONTRASENA_USU VARCHAR(500) NOT NULL)

/*PROCEDIMIENTO ALMACENADO PARA CREAR REGISTROS DE LA TABLA USUARIOS*/
CREATE PROCEDURE REGISTRAR_USUARIOS(
@NOMBRE_COMPLETO_USU VARCHAR(100),
@CELULAR_USU VARCHAR(20),
@ID_CIUDAD_FK INT,
@CORREO_ELECTRONICO_USU VARCHAR(100),
@CONTRASENA_USU VARCHAR(500),
@REGISTRADO BIT OUTPUT,
@MENSAJE VARCHAR(100) OUTPUT)
AS 
BEGIN
	IF(NOT EXISTS(SELECT * FROM USUARIOS WHERE CORREO_ELECTRONICO_USU = @CORREO_ELECTRONICO_USU OR CELULAR_USU = @CELULAR_USU))
	BEGIN 
		INSERT INTO USUARIOS
		VALUES (@NOMBRE_COMPLETO_USU,@CELULAR_USU,@ID_CIUDAD_FK,@CORREO_ELECTRONICO_USU,@CONTRASENA_USU)
		SET @REGISTRADO = 1
		SET @MENSAJE = 'Usuario registrado. Inicia sesión para continuar.'
	END
	ELSE
	BEGIN
		SET @REGISTRADO = 0
		SET @MENSAJE = '* El correo electrónico o el número de teléfono celular ya están en uso'
	END
END

/*PROCEDIMIENTO ALMACENADO PARA LEER REGISTROS DE LA TABLA USUARIOS*/
CREATE PROCEDURE LEER_USUARIOS
AS
BEGIN
SELECT ID_USUARIO, NOMBRE_COMPLETO_USU, CELULAR_USU, ID_CIUDAD_FK, NOMBRE_CIUDAD, CORREO_ELECTRONICO_USU
    FROM USUARIOS
	INNER JOIN CIUDADES
	ON USUARIOS.ID_CIUDAD_FK = CIUDADES.ID_CIUDAD
END 

EXEC LEER_USUARIOS

/*PROCEDIMIENTO ALMACENADO PARA ACTUALIZAR REGISTROS DE LA TABLA USUARIOS*/
CREATE PROCEDURE ACTUALIZAR_USUARIO(
@ID_USUARIO INT,
@NOMBRE_COMPLETO_USU VARCHAR(100),
@CELULAR_USU VARCHAR(20),
@ID_CIUDAD_FK INT,
@CORREO_ELECTRONICO_USU VARCHAR(100),
@MENSAJE VARCHAR(100) OUTPUT)
AS
BEGIN
	IF EXISTS (
        SELECT *
        FROM USUARIOS
        WHERE (CORREO_ELECTRONICO_USU = @CORREO_ELECTRONICO_USU OR CELULAR_USU = @CELULAR_USU)
          AND ID_USUARIO <> @ID_USUARIO
    )
    BEGIN
        SET @MENSAJE = 'El correo electrónico o el número de celular ya están registrados. Inténtelo nuevamente.'
    END
	ELSE
    BEGIN
		UPDATE USUARIOS 
		SET NOMBRE_COMPLETO_USU = @NOMBRE_COMPLETO_USU, CELULAR_USU = @CELULAR_USU, ID_CIUDAD_FK = @ID_CIUDAD_FK, CORREO_ELECTRONICO_USU = @CORREO_ELECTRONICO_USU
		WHERE ID_USUARIO= @ID_USUARIO
	END
END

/*PROCEDIMIENTO ALMACENADO PARA BORRAR REGISTROS DE LA TABLA USUARIOS */
CREATE PROCEDURE BORRAR_USUARIOS(
@ID_USUARIO INT)
AS
BEGIN
	DELETE FROM USUARIOS WHERE ID_USUARIO = @ID_USUARIO
END

/*PROCEDIMIENTO ALMACENADO PARA VALIDAR USUARIOS*/
CREATE PROCEDURE VALIDAR_USUARIO(
@CORREO_ELECTRONICO_USU VARCHAR(100),
@CONTRASENA_USU VARCHAR(500))
AS
BEGIN
	IF(EXISTS(SELECT * FROM USUARIOS WHERE CORREO_ELECTRONICO_USU = @CORREO_ELECTRONICO_USU AND CONTRASENA_USU = @CONTRASENA_USU))
		SELECT ID_USUARIO, NOMBRE_COMPLETO_USU, CELULAR_USU, ID_CIUDAD_FK, CORREO_ELECTRONICO_USU, CONTRASENA_USU
		FROM USUARIOS WHERE CORREO_ELECTRONICO_USU = @CORREO_ELECTRONICO_USU AND CONTRASENA_USU = @CONTRASENA_USU
	ELSE
		SELECT '0' AS ID_USUARIO, '' AS NOMBRE_COMPLETO_USU, '' AS CELULAR_USU, 0 AS ID_CIUDAD_FK, '' AS CORREO_ELECTRONICO_USU, '' AS CONTRASENA_USU
END

EXEC VALIDAR_USUARIO 'arangomoni15@gmail.com','MQAyADMA'

/*PROCEDIMIENTO ALMACENADO PARA VISUALIZAR LA INFORMACION DE UN USUARIO*/
CREATE PROCEDURE INFORMACION_USUARIO(
@ID_USUARIO INT)
AS
BEGIN
    SELECT ID_USUARIO, NOMBRE_COMPLETO_USU, CELULAR_USU, ID_CIUDAD_FK, NOMBRE_CIUDAD, CORREO_ELECTRONICO_USU
    FROM USUARIOS
	INNER JOIN CIUDADES
	ON USUARIOS.ID_CIUDAD_FK = CIUDADES.ID_CIUDAD
    WHERE ID_USUARIO = @ID_USUARIO
END

EXEC INFORMACION_USUARIO 1

/*PROCEDIMIENTO ALMACENADO PARA CAMBIAR LA CONTRASEÑA DE UN USUARIO*/
CREATE PROCEDURE CAMBIAR_CONTRASENA
    @ID_USUARIO INT,
    @CONTRASENA_ACTUAL VARCHAR(500),
    @CONTRASENA_NUEVA VARCHAR(500),
	@MENSAJE_C VARCHAR(100) OUTPUT 
AS
BEGIN
    IF EXISTS (SELECT * FROM USUARIOS WHERE ID_USUARIO = @ID_USUARIO AND CONTRASENA_USU = @CONTRASENA_ACTUAL)
    BEGIN
        UPDATE USUARIOS SET CONTRASENA_USU = @CONTRASENA_NUEVA WHERE ID_USUARIO = @ID_USUARIO
        SET @MENSAJE_C = 'Contraseña actualizada correctamente'
    END
    ELSE
    BEGIN
       SET @MENSAJE_C = 'La contraseña actual ingresada no coincide'
    END
END

/*CREACION DE LA FUNCION PARA CONTAR EL NUMERO DE USUARIOS REGISTRADOS*/
CREATE FUNCTION NUMERO_USUARIOS()
RETURNS INT
AS
BEGIN
  DECLARE @TOTAL_USUARIOS INT;

  SELECT @TOTAL_USUARIOS = COUNT(*) FROM USUARIOS;

  RETURN @TOTAL_USUARIOS;
END;

SELECT dbo.NUMERO_USUARIOS() AS NumeroUsuarios;

/*REINICIAR CONTADOR DEL ID*/
SELECT MAX(ID_USUARIO) FROM USUARIOS;
DBCC CHECKIDENT (USUARIOS, RESEED, 11)

-----------------------------------------------------------------------------------------------------------------------

/*CREACION DE LA TABLA CATEGORIAS*/
CREATE TABLE CATEGORIAS(
ID_CATEGORIA INT IDENTITY(1,1) PRIMARY KEY,
NOMBRE_CAT VARCHAR(MAX) NOT NULL,
DESCRIPCION_CAT VARCHAR(MAX) NOT NULL)

/*PROCEDIMIENTO ALMACENADO PARA CREAR REGISTROS DE LA TABLA CATEGORIAS*/
CREATE PROCEDURE CREAR_CATEGORIAS(
@NOMBRE_CAT VARCHAR(MAX),
@DESCRIPCION_CAT VARCHAR(MAX),
@MENSAJE_CATEGORIAS VARCHAR(100) OUTPUT)
AS
BEGIN
	IF EXISTS (SELECT * FROM CATEGORIAS WHERE NOMBRE_CAT = @NOMBRE_CAT)
    BEGIN
       SET @MENSAJE_CATEGORIAS = 'La categoría "' + @NOMBRE_CAT + '" ya se encuentra registrada.'
    END
    ELSE
    BEGIN
	INSERT INTO CATEGORIAS(NOMBRE_CAT,DESCRIPCION_CAT) VALUES (@NOMBRE_CAT,@DESCRIPCION_CAT)
    END
END

/*PROCEDIMIENTO ALMACENADO PARA LEER REGISTROS DE LA TABLA CATEGORIAS */
CREATE PROCEDURE LEER_CATEGORIAS
AS
BEGIN
	SELECT * FROM CATEGORIAS 
END

EXEC LEER_CATEGORIAS

/*PROCEDIMIENTO ALMACENADO PARA ACTUALIZAR REGISTROS DE LA TABLA CATEGORIAS */
CREATE PROCEDURE ACTUALIZAR_CATEGORIAS(
@ID_CATEGORIA INT,
@NOMBRE_CAT VARCHAR(MAX),
@DESCRIPCION_CAT VARCHAR(MAX),
@MENSAJE_CATEGORIAS_ACT VARCHAR(100) OUTPUT)
AS
BEGIN
	IF NOT EXISTS (SELECT * FROM CATEGORIAS WHERE NOMBRE_CAT = @NOMBRE_CAT AND ID_CATEGORIA <> @ID_CATEGORIA)
    BEGIN
        UPDATE CATEGORIAS SET NOMBRE_CAT = @NOMBRE_CAT, DESCRIPCION_CAT = @DESCRIPCION_CAT WHERE ID_CATEGORIA = @ID_CATEGORIA
        SET @MENSAJE_CATEGORIAS_ACT = 'La categoría ha sido actualizada exitosamente.';
    END
    ELSE
    BEGIN
        SET @MENSAJE_CATEGORIAS_ACT = 'El nombre de la categoría ya está en uso.';
    END	
END

/*PROCEDIMIENTO ALMACENADO PARA BORRAR REGISTROS DE LA TABLA CATEGORIAS */
CREATE PROCEDURE BORRAR_CATEGORIAS(
@ID_CATEGORIA INT)
AS
BEGIN
	DELETE FROM CATEGORIAS WHERE ID_CATEGORIA = @ID_CATEGORIA
END

/*CREACION DE LA FUNCION PARA CONTAR EL NUMERO DE CATEGORIAS REGISTRADAS*/
CREATE FUNCTION NUMERO_CATEGORIAS()
RETURNS INT
AS
BEGIN
  DECLARE @TOTAL_CATEGORIAS INT;

  SELECT @TOTAL_CATEGORIAS = COUNT(*) FROM CATEGORIAS;

  RETURN @TOTAL_CATEGORIAS;
END;

SELECT dbo.NUMERO_CATEGORIAS() AS NumeroCategorias;

/*REINICIAR CONTADOR DEL ID*/
SELECT MAX(ID_CATEGORIA) FROM CATEGORIAS;
DBCC CHECKIDENT (CATEGORIAS, RESEED, 4)

-----------------------------------------------------------------------------------------------------------------------

/*CREACION DE LA TABLA ADMINISTRADORES*/
CREATE TABLE ADMINISTRADORES(
ID_ADMINISTRADOR INT PRIMARY KEY IDENTITY(1,1),
NOMBRE_COMPLETO_ADMIN VARCHAR(100),
CORREO_ELECTRONICO_ADMIN VARCHAR(100) NOT NULL,
CONTRASENA_ADMIN VARCHAR(500) NOT NULL)

/*PROCEDIMIENTO ALMACENADO PARA CREAR REGISTROS DE LA TABLA ADMINISTRADORES*/
CREATE PROCEDURE REGISTRAR_ADMINISTRADORES(
@NOMBRE_COMPLETO_ADMIN VARCHAR(100),
@CORREO_ELECTRONICO_ADMIN VARCHAR(100),
@CONTRASENA_ADMIN VARCHAR(500),
@REGISTRADO BIT OUTPUT,
@MENSAJE VARCHAR(100) OUTPUT)
AS 
BEGIN
	IF(NOT EXISTS(SELECT * FROM ADMINISTRADORES WHERE CORREO_ELECTRONICO_ADMIN = @CORREO_ELECTRONICO_ADMIN))
	BEGIN 
		INSERT INTO ADMINISTRADORES
		VALUES (@NOMBRE_COMPLETO_ADMIN,@CORREO_ELECTRONICO_ADMIN,@CONTRASENA_ADMIN)
		SET @REGISTRADO = 1
		SET @MENSAJE = 'Administrador registrado'
	END
	ELSE
	BEGIN
		SET @REGISTRADO = 0
		SET @MENSAJE = '* El correo electrónico ya están en uso'
	END
END

/*PROCEDIMIENTO ALMACENADO PARA LEER REGISTROS DE LA TABLA ADMINISTRADORES*/
CREATE PROCEDURE LEER_ADMINISTRADORES
AS
BEGIN
	SELECT * FROM ADMINISTRADORES
END 

EXEC LEER_ADMINISTRADORES

/*PROCEDIMIENTO ALMACENADO PARA ACTUALIZAR REGISTROS DE LA TABLA ADMINISTRADORES*/
CREATE PROCEDURE ACTUALIZAR_ADMINISTRADORES(
@ID_ADMINISTRADOR INT,
@NOMBRE_COMPLETO_ADMIN VARCHAR(100),
@CORREO_ELECTRONICO_ADMIN VARCHAR(100),
@CONTRASENA_ADMIN VARCHAR(500))
AS
BEGIN
	UPDATE ADMINISTRADORES 
	SET NOMBRE_COMPLETO_ADMIN = @NOMBRE_COMPLETO_ADMIN, CORREO_ELECTRONICO_ADMIN = @CORREO_ELECTRONICO_ADMIN, CONTRASENA_ADMIN = @CONTRASENA_ADMIN
	WHERE ID_ADMINISTRADOR= @ID_ADMINISTRADOR
END

/*PROCEDIMIENTO ALMACENADO PARA BORRAR REGISTROS DE LA TABLA ADMINISTRADORES */
CREATE PROCEDURE BORRAR_ADMINISTRADORES(
@ID_ADMINISTRADOR INT)
AS
BEGIN
	DELETE FROM ADMINISTRADORES WHERE ID_ADMINISTRADOR = @ID_ADMINISTRADOR
END

/*PROCEDIMIENTO ALMACENADO PARA VALIDAR ADMINISTRADOR*/
CREATE PROCEDURE VALIDAR_ADMINISTRADOR(
@CORREO_ELECTRONICO_ADMIN VARCHAR(100),
@CONTRASENA_ADMIN VARCHAR(500))
AS
BEGIN
	IF(EXISTS(SELECT * FROM ADMINISTRADORES WHERE CORREO_ELECTRONICO_ADMIN = @CORREO_ELECTRONICO_ADMIN AND CONTRASENA_ADMIN = @CONTRASENA_ADMIN))
		SELECT ID_ADMINISTRADOR, NOMBRE_COMPLETO_ADMIN, CORREO_ELECTRONICO_ADMIN, CONTRASENA_ADMIN 
		FROM ADMINISTRADORES WHERE CORREO_ELECTRONICO_ADMIN = @CORREO_ELECTRONICO_ADMIN AND CONTRASENA_ADMIN = @CONTRASENA_ADMIN
	ELSE
		SELECT '0' AS ID_ADMINISTRADOR, '' AS NOMBRE_COMPLETO_ADMIN, '' AS CORREO_ELECTRONICO_ADMIN, '' AS CONTRASENA_ADMIN
END

/*CREACION DE LA FUNCION PARA CONTAR EL NUMERO DE ADMINISTRADORES REGISTRADOS*/
CREATE FUNCTION NUMERO_ADMINISTRADORES()
RETURNS INT
AS
BEGIN
  DECLARE @TOTAL_ADMINISTRADORES INT;

  SELECT @TOTAL_ADMINISTRADORES = COUNT(*) FROM ADMINISTRADORES;

  RETURN @TOTAL_ADMINISTRADORES;
END;

SELECT dbo.NUMERO_ADMINISTRADORES() AS NumeroAdministradores;

/*REINICIAR CONTADOR DEL ID*/
SELECT MAX(ID_ADMINISTRADOR) FROM ADMINISTRADORES;
DBCC CHECKIDENT (ADMINISTRADORES, RESEED, 1)

-----------------------------------------------------------------------------------------------------------------------

/*CREACION DE LA TABLA SERVICIOS*/
CREATE TABLE SERVICIOS(
ID_SERVICIO INT IDENTITY(1,1) PRIMARY KEY,
NOMBRE_SER VARCHAR(70) NOT NULL,
PRECIO_SER DECIMAL NOT NULL,
DESCRIPCION_BREVE VARCHAR(500) NOT NULL,
TERMINOS_SER VARCHAR(MAX) NOT NULL,
ESTADO_DS VARCHAR(20) DEFAULT 'Activo',
TIPO VARCHAR(50) NOT NULL,
FECHA_PUBLICACION DATE NOT NULL DEFAULT GETDATE(),
ID_USUARIO_FK INT REFERENCES USUARIOS(ID_USUARIO),
ID_ADMINISTRADOR_FK INT REFERENCES ADMINISTRADORES(ID_ADMINISTRADOR),
ID_CATEGORIA_FK INT REFERENCES CATEGORIAS(ID_CATEGORIA))

/*PROCEDIMIENTO ALMACENADO PARA CREAR REGISTROS DE LA TABLA SERVICIOS*/
CREATE PROCEDURE CREAR_SERVICIOS(
@NOMBRE_SER VARCHAR(70),
@PRECIO_SER DECIMAL,
@DESCRIPCION_BREVE VARCHAR(500),
@TERMINOS_SER VARCHAR(MAX),
@TIPO VARCHAR(50),
@ID_USUARIO_FK INT,
@ID_CATEGORIA_FK INT
)
AS 
BEGIN
	INSERT INTO SERVICIOS(NOMBRE_SER,PRECIO_SER,DESCRIPCION_BREVE,TERMINOS_SER,TIPO,ID_USUARIO_FK,ID_CATEGORIA_FK) 
	VALUES (@NOMBRE_SER,@PRECIO_SER,@DESCRIPCION_BREVE,@TERMINOS_SER,@TIPO,@ID_USUARIO_FK,@ID_CATEGORIA_FK)
END

/*PROCEDIMIENTO ALMACENADO PARA LEER REGISTROS DE LA TABLA SERVICIOS */
CREATE PROCEDURE LEER_SERVICIOS
AS
BEGIN
	SELECT ID_SERVICIO, NOMBRE_SER, PRECIO_SER, DESCRIPCION_BREVE, TERMINOS_SER, ESTADO_DS, TIPO, FECHA_PUBLICACION, NOMBRE_CAT, ID_USUARIO_FK
	FROM SERVICIOS INNER JOIN CATEGORIAS 
	ON SERVICIOS.ID_CATEGORIA_FK = CATEGORIAS.ID_CATEGORIA
END

EXEC LEER_SERVICIOS

/*PROCEDIMIENTO ALMACENADO PARA ACTUALIZAR REGISTROS DE LA TABLA SERVICIOS */
CREATE PROCEDURE ACTUALIZAR_SERVICIO(
@ID_SERVICIO INT,
@NOMBRE_SER VARCHAR(70),
@PRECIO_SER DECIMAL,
@DESCRIPCION_BREVE VARCHAR(500),
@TERMINOS_SER VARCHAR(MAX),
@ID_CATEGORIA_FK INT,
@TIPO VARCHAR(50))
AS
BEGIN
	UPDATE SERVICIOS SET NOMBRE_SER = @NOMBRE_SER, PRECIO_SER = @PRECIO_SER, DESCRIPCION_BREVE = @DESCRIPCION_BREVE, TERMINOS_SER = @TERMINOS_SER, ID_CATEGORIA_FK = @ID_CATEGORIA_FK 
	WHERE ID_SERVICIO = @ID_SERVICIO AND TIPO = @TIPO
END

/*PROCEDIMIENTO ALMACENADO PARA BORRAR REGISTROS DE LA TABLA SERVICIOS */
CREATE PROCEDURE BORRAR_SERVICIOS(
@ID_SERVICIO INT)
AS
BEGIN
	DELETE FROM SERVICIOS WHERE ID_SERVICIO = @ID_SERVICIO
	UPDATE HISTORIAL_SERVICIOS SET ESTADO_DS = 'Desactivo' WHERE ID_SERVICIO = @ID_SERVICIO
END

/*PROCEDIMIENTO ALMACENADO PARA CONSULTAR PUBLICACIONES Y SOLICITUDES DE SERVICIOS INNER JOIN CON TABLA CATEGORIAS*/
CREATE PROCEDURE CONSULTAR_SERVICIOS
AS 
BEGIN
	SELECT ID_SERVICIO, NOMBRE_SER, PRECIO_SER, DESCRIPCION_BREVE, NOMBRE_CAT, TIPO
	FROM SERVICIOS INNER JOIN CATEGORIAS 
	ON SERVICIOS.ID_CATEGORIA_FK = CATEGORIAS.ID_CATEGORIA
	ORDER BY FECHA_PUBLICACION DESC
END

EXEC CONSULTAR_SERVICIOS

/*PROCEDIMIENTO ALMACENADO PARA LA BUSQUEDA DE SERVICIOS*/
CREATE PROCEDURE BUSQUEDA_COMPLETA(
@NOMBRE_SER VARCHAR(70),
@TIPO VARCHAR(50) = NULL)
AS
BEGIN
    IF @TIPO IS NULL
    BEGIN
        -- Buscar tanto publicaciones como solicitudes
        SELECT ID_SERVICIO, NOMBRE_SER, PRECIO_SER, DESCRIPCION_BREVE, NOMBRE_CAT, TIPO
        FROM SERVICIOS 
        INNER JOIN CATEGORIAS ON SERVICIOS.ID_CATEGORIA_FK = CATEGORIAS.ID_CATEGORIA
        WHERE NOMBRE_SER LIKE '%'+@NOMBRE_SER+'%'
		ORDER BY FECHA_PUBLICACION DESC
    END
    ELSE IF @TIPO = 'Publicacion'
    BEGIN
        -- Buscar solo publicaciones
        SELECT ID_SERVICIO, NOMBRE_SER, PRECIO_SER, DESCRIPCION_BREVE, NOMBRE_CAT, TIPO
        FROM SERVICIOS 
        INNER JOIN CATEGORIAS ON SERVICIOS.ID_CATEGORIA_FK = CATEGORIAS.ID_CATEGORIA
        WHERE NOMBRE_SER LIKE '%'+@NOMBRE_SER+'%' AND TIPO = 'Publicacion'
		ORDER BY FECHA_PUBLICACION DESC
    END
    ELSE IF @TIPO = 'Solicitud'
    BEGIN
        -- Buscar solo solicitudes
        SELECT ID_SERVICIO, NOMBRE_SER, PRECIO_SER, DESCRIPCION_BREVE, NOMBRE_CAT, TIPO 
        FROM SERVICIOS 
        INNER JOIN CATEGORIAS ON SERVICIOS.ID_CATEGORIA_FK = CATEGORIAS.ID_CATEGORIA
        WHERE NOMBRE_SER LIKE '%'+@NOMBRE_SER+'%' AND TIPO = 'Solicitud'
		ORDER BY FECHA_PUBLICACION DESC
    END
END

/*PROCEDIMIENTO ALMACENADO PARA CATEGORIZAR LOS SERVICIOS*/
CREATE PROCEDURE CATEGORIAS_SERVICIOS(
  @ID_CATEGORIA INT,
  @TIPO VARCHAR(50)
)
AS
BEGIN
  SELECT ID_SERVICIO, NOMBRE_SER, PRECIO_SER, DESCRIPCION_BREVE, NOMBRE_CAT, TIPO
  FROM SERVICIOS
  INNER JOIN CATEGORIAS ON SERVICIOS.ID_CATEGORIA_FK = CATEGORIAS.ID_CATEGORIA
  WHERE ID_CATEGORIA = @ID_CATEGORIA
    AND
    (
      (@TIPO = 'Publicacion' AND TIPO = 'Publicacion')
      OR
      (@TIPO = 'Solicitud' AND TIPO = 'Solicitud')
      OR
      (@TIPO = 'Ambos')
    )
	 ORDER BY FECHA_PUBLICACION DESC
END

/*PROCEDIMIENTO ALMACENADO PARA CONSULTAR PUBLICACIONES O SOLICITUDES DE SERVICIOS INNER JOIN CON TABLA CATEGORIAS*/
CREATE PROCEDURE CONSULTAR_PUBLICACIONES_SOLICITUDES
    @TIPO VARCHAR(50)
AS 
BEGIN
    IF @TIPO  = 'Publicacion'
    BEGIN
        SELECT ID_SERVICIO, NOMBRE_SER, PRECIO_SER, DESCRIPCION_BREVE, NOMBRE_CAT, TIPO
        FROM SERVICIOS 
        INNER JOIN CATEGORIAS ON SERVICIOS.ID_CATEGORIA_FK = CATEGORIAS.ID_CATEGORIA 
        WHERE TIPO = 'Publicacion'
		ORDER BY FECHA_PUBLICACION DESC
    END
    ELSE IF @TIPO  = 'Solicitud'
    BEGIN
        SELECT ID_SERVICIO, NOMBRE_SER, PRECIO_SER, DESCRIPCION_BREVE, NOMBRE_CAT, TIPO
        FROM SERVICIOS 
        INNER JOIN CATEGORIAS ON SERVICIOS.ID_CATEGORIA_FK = CATEGORIAS.ID_CATEGORIA 
        WHERE TIPO = 'Solicitud'
		ORDER BY FECHA_PUBLICACION DESC
    END
END

/*VISTA MAS INFORMACION SOBRE UN SERVICIO
TABLAS: SERVICIOS // CATEGORIAS // USUARIOS // CIUDADES*/
CREATE VIEW MAS_INFORMACION
AS
SELECT ID_USUARIO, NOMBRE_COMPLETO_USU, CELULAR_USU, NOMBRE_CIUDAD,
ID_SERVICIO, NOMBRE_SER, PRECIO_SER, DESCRIPCION_BREVE, TERMINOS_SER, ESTADO_DS, TIPO, NOMBRE_CAT
FROM CIUDADES INNER JOIN USUARIOS 
ON CIUDADES.ID_CIUDAD = USUARIOS.ID_CIUDAD_FK 
INNER JOIN SERVICIOS
ON USUARIOS.ID_USUARIO = SERVICIOS.ID_USUARIO_FK
INNER JOIN CATEGORIAS 
ON SERVICIOS.ID_CATEGORIA_FK = CATEGORIAS.ID_CATEGORIA

/*PROCEDIMIENTO ALMACENADO PARA CONSULTAR VISTA MAS INFORMACION*/
CREATE PROCEDURE INFORMACION_SERVICIO(
    @ID_SERVICIO INT,
    @TIPO VARCHAR(50)
)
AS
BEGIN
    IF @TIPO = 'Publicacion'
    BEGIN
        SELECT * FROM MAS_INFORMACION WHERE ID_SERVICIO = @ID_SERVICIO AND TIPO = 'Publicacion'
    END
    ELSE IF @TIPO = 'Solicitud'
    BEGIN
        SELECT * FROM MAS_INFORMACION WHERE ID_SERVICIO = @ID_SERVICIO AND TIPO = 'Solicitud'
    END
END

/*PROCEDIMIENTO ALMACENADO PARA CONSULTAR SERVICIOS PUBLICADOS POR UN USUARIO INNER JOIN CON TABLA CATEGORIAS*/
CREATE PROCEDURE SERVICIOS_USUARIO
    @ID_USUARIO INT,
	@TIPO VARCHAR(50)
AS 
BEGIN
    SELECT ID_SERVICIO, NOMBRE_SER, PRECIO_SER, DESCRIPCION_BREVE, NOMBRE_CAT, TIPO
    FROM SERVICIOS 
    INNER JOIN CATEGORIAS ON SERVICIOS.ID_CATEGORIA_FK = CATEGORIAS.ID_CATEGORIA
    WHERE SERVICIOS.ID_USUARIO_FK = @ID_USUARIO AND TIPO = @TIPO
    ORDER BY FECHA_PUBLICACION DESC
END

EXEC SERVICIOS_USUARIO 1,'Solicitud'

/*CREACION DE LA FUNCION PARA CONTAR EL NUMERO DE SERVICIOS REGISTRADOS*/
CREATE FUNCTION NUMERO_SERVICIOS()
RETURNS INT
AS
BEGIN
  DECLARE @TOTAL_SERVICIOS INT;

  SELECT @TOTAL_SERVICIOS = COUNT(*) FROM SERVICIOS;

  RETURN @TOTAL_SERVICIOS;
END;

SELECT dbo.NUMERO_SERVICIOS() AS NumeroServicios;

/*REINICIAR CONTADOR DEL ID*/
DBCC CHECKIDENT (SERVICIOS, RESEED, 0)

-----------------------------------------------------------------------------------------------------------------------

/*CREACION DE LA TABLA HISTORIAL DE SERVICIOS ELIMINADOS*/
CREATE TABLE [HISTORIAL_SERVICIOS](
	[ID_SERVICIO] [int] NOT NULL,
	[NOMBRE_SER] [varchar](70) NOT NULL,
	[PRECIO_SER] [decimal] NOT NULL,
	[DESCRIPCION_BREVE] [varchar](500) NOT NULL,
	[TERMINOS_SER] [varchar](max) NOT NULL,
	[ESTADO_DS] [varchar](20) NULL,
	[TIPO] [varchar](50) NOT NULL,
	[FECHA_PUBLICACION] [date] NOT NULL,
	[ID_USUARIO_FK] [int] NULL,
	[ID_ADMINISTRADOR_FK] [int] NULL,
	[ID_CATEGORIA_FK] [int] NULL)

/*CREACION DE DISPARADOR PARA QUE AGREGUE SERVICIOS ELIMINADOS AL HISTORIAL*/
CREATE TRIGGER TR_HISTORIAL_SERVICIOS
ON SERVICIOS FOR DELETE
AS 
BEGIN 
	INSERT INTO [HISTORIAL_SERVICIOS]
	SELECT * FROM deleted
END 
GO

/*PROCEDIMIENTO ALMACENADO PARA CONSULTAR EL HISTORIAL DE SERVICIOS PUBLICADOS POR UN USUARIO INNER JOIN CON TABLA CATEGORIAS*/
CREATE PROCEDURE HISTORIAL_USUARIO
    @ID_USUARIO INT,
	@TIPO VARCHAR(50)
AS 
BEGIN
    SELECT ID_SERVICIO, NOMBRE_SER, PRECIO_SER, DESCRIPCION_BREVE, TIPO, ESTADO_DS, NOMBRE_CAT
    FROM HISTORIAL_SERVICIOS 
    INNER JOIN CATEGORIAS ON HISTORIAL_SERVICIOS.ID_CATEGORIA_FK = CATEGORIAS.ID_CATEGORIA
    WHERE HISTORIAL_SERVICIOS.ID_USUARIO_FK = @ID_USUARIO AND TIPO = @TIPO
    ORDER BY FECHA_PUBLICACION DESC
END

EXEC HISTORIAL_USUARIO 1,'Solicitud'
